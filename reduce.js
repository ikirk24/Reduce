...
function extractValue(arr, key) {
    return arr.reduce(function(accumulator, index){
        if (!index[key]) {
            return accumulator;
        } accumulator.push(index[key])
        return accumulator;
    }, [])
}
...
function addKeyAndValue(arr, key, value) {
    return arr.reduce(function(accum, currentVal, idx){
        accum[idx][key] = value;
        return accum; 
    }, arr);
}
...
function partition(arr, callback) {
    return arr.reduce(function(accum, currentVal){
        if (callback(currentVal)){
            accum[0].push(currentVal)
        } else {
            accum[1].push(currentVal); 
        }
        return accum; 
    }, [[],[]]);
}
